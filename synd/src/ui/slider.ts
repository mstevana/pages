...
export class SectionSlider {
  ...
  levelAt(py: number, gm: SliderGeom): number {
    ...
    return Math.round(Math.max(0, Math.min(1, f)) * this.maxLevel);
  }

  draw(g: CanvasRenderingContext2D, gm: SliderGeom, level: number): void {
    const off = level >= this.maxLevel;
    const f = this.maxLevel > 0 ? level / this.maxLevel : 1;
    ...
    const step = Math.max(1, Math.ceil(this.maxLevel / 16));
    for (let l = 0; l <= this.maxLevel; l += step) {
      const y = gm.y + (1 - l / this.maxLevel) * gm.h;
